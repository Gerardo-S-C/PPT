...
socket.on("player-1-wins", ({myChoice, enemyChoice}) => {
    if(playerId === 1){
        let message = "Elegiste " + myChoice + " y el enemigo eligió " + enemyChoice + " . GANASTE!";
        setWinningMessage(message);
        myScorePoints++;
    }else{
        let message = "Elegiste " + myChoice + " y el enemigo eligió " + enemyChoice + " . PERDISTE!";
        setWinningMessage(message);
        enemyScorePoints++;
    }
...
function setPlayerTag(playerId){
    if(playerId === 1){
        playerOneTag.innerText = "Yo (Jugador 1)";
        playerTwoTag.innerText = "Enemigo (Jugador 2)";
    }else{
        playerOneTag.innerText = "Enemigo (Jugador 2)";
        playerTwoTag.innerText = "Yo (Jugador 1)";
    }
}
...
function setWaitMessage(display){
    if(display){
        let p = document.createElement("p");
        p.innerText = "Esperando a que otro jugador se una...";
...
function choose(choice){
    if(choice === "roca"){
        roca.classList.add("my-choice");
    }else if(choice === "papel"){
        papel.classList.add("my-choice");
    }else{
        tijeras.classList.add("my-choice");
    }
...
function removeChoice(choice){
    if(choice === "roca"){
        roca.classList.remove("my-choice");
    }else if(choice === "papel"){
        papel.classList.remove("my-choice");
    }else{
        tijeras.classList.remove("my-choice");
    }
